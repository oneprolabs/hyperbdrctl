...
func (s Service) Wait(spec WaitSpec) (WaitExecutionResult, error) {
	...
	row := s.waitForObjectStorage(spec.ID, spec.Interval, spec.Timeout)
...
func (s Service) waitForObjectStorage(storageID string, interval, timeout time.Duration) map[string]interface{} {
	start := time.Now()
	...
	for {
		...
		if err != nil {
			return waitutil.WaitResult(storageID, "create-oss", "failed", "", "", elapsed, err.Error())
		}
		if notFound {
			return waitutil.WaitResult(storageID, "create-oss", "failed", "not_found", "", elapsed, "object storage not found")
		}

		if actualType := storageKind(storage); actualType != "" && actualType != "objectstorage" {
			errText := fmt.Sprintf("resource type mismatch: expected objectstorage, got %s", actualType)
			return waitutil.WaitResult(storageID, "create-oss", "failed", waitutil.FirstNonEmptyString(storage["status"]), waitutil.FirstNonEmptyString(storage["display_status"], storage["displayStatus"]), elapsed, errText)
		}
		...
		case waitutil.IsFailureStatus(status, displayStatus):
			return waitutil.WaitResult(storageID, "create-oss", "failed", status, displayStatus, elapsed, "")
		case waitutil.IsSuccessStatus(status, displayStatus):
			return waitutil.WaitResult(storageID, "create-oss", "success", status, displayStatus, elapsed, "")
		}

		if !time.Now().Before(deadline) {
			return waitutil.WaitResult(storageID, "create-oss", "timeout", status, displayStatus, elapsed, "wait timed out")
		}
		if interval > 0 {
			time.Sleep(interval)
		}
...
func (s Service) fetchStorageDetail(storageID string) (map[string]interface{}, bool, error) {
	...
	if data == nil {
		return nil, false, fmt.Errorf("object storage detail response is empty")
	}
