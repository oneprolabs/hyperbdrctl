...
func TestServiceWaitCloudSyncGatewaySucceedsAfterPolling(t *testing.T) {
	api := &fakeWaitAPI{
		responses: []client.APIResponse{
			{Data: map[string]interface{}{"storage": map[string]interface{}{"type": "HyperGate", "status": "creating"}}},
			{Data: map[string]interface{}{"storage": map[string]interface{}{"type": "HyperGate", "status": "available", "display_status": "Available"}}},
		},
	}
	...
	row := result.Rows[0]
	if row["result"] != "success" || row["operation"] != "create-cloud-sync-gateway" {
		t.Fatalf("row = %+v", row)
	}
}

func TestServiceWaitCloudSyncGatewayTypeMismatchFails(t *testing.T) {
	api := &fakeWaitAPI{
		responses: []client.APIResponse{
	...
	if row["error"] != "resource type mismatch: expected cloud-sync-gateway, got objectstorage" {
		t.Fatalf("row = %+v", row)
	}
}
